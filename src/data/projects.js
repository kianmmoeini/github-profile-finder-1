const projects = [
  {
    id: 1,
    title: "GitHub Profile Finder",
    description:
      "A modern GitHub profile finder application that allows users to search for GitHub profiles and explore their repositories.",
    image: "/github-project-1.png",
    technologies: ["React", "React Query", "Axios", "MUI"],
    github: "https://github.com/kianmmoeini/github-profile-finder-1",
    demo: "https://github-profile-finder-project-lemon.vercel.app/",
  },

  {
    id: 2,
    title: "Coming Soon",
    description:
      "A new project built with modern frontend technologies. More details coming soon.",
    image: "/github-project.png",
    technologies: ["React", "JavaScript"],
    github: "https://github.com/kianmmoeini",
    demo: "https://github-profile-finder-project-lemon.vercel.app/",
  },
];

export default projects;